...
from ultimate_rvc.web.guided_training import (
    bind_guided_training,
    render_guided_controls,
)
...
def render(total_config: TotalConfig) -> None:
    ...
    """
    with gr.Tab("Guided training"):
        guided_controls = render_guided_controls()
        _render_step_1(total_config)
        _render_step_2(total_config)
        _render_step_3(total_config)
        bind_guided_training(guided_controls, total_config.training.multi_step)
...
def _list_sliced_audio(model_name: str) -> gr.Dropdown:
    ...
    if not sliced_dir.is_dir():
        raise gr.Error(
            f"No sliced audio found for model '{model_name}'. "
    ...
    return gr.Dropdown(
        choices=choices,
        value=str(files[0]),
        label=f"Sliced clips ({len(files)} total)",
    )
...
def _render_step_2(total_config: TotalConfig) -> None:
    tab_config = total_config.training.multi_step
    with gr.Accordion("Step 2: Extract voice features", open=True):
        with gr.Row():
...
def _render_step_3(total_config: TotalConfig) -> None:
    tab_config = total_config.training.multi_step
    with gr.Accordion("Step 3: Train and export model"):
        with gr.Row():
